...
get_sa_species = function(
  ...
  if (type == 'auto' | type == 'all'){
    # If Auto data exists
    if (length(unique(sa_bulk_df$auto_id)) > 1 | (length(unique(sa_bulk_df$auto_id)) == 1 & !is.na(unique(sa_bulk_df$auto_id))[1])){
      # Auto Species
  ...
  if (type == 'man' | type == 'all'){
    ...
    if (length(unique(sa_bulk_df$manual_id)) > 1 | (length(unique(sa_bulk_df$manual_id)) == 1 & !is.na(unique(sa_bulk_df$manual_id))[1])){
      # Manual Species
...
get_sa_range_maps = function(
  ...
  # Merge the two together (key = species) Note: Myotis melanorhinus exists in shapefile but
  ...
  for (spc in proj_species){
    if (spc %in% species_range_df$species_code){
      ...
      m = m %>% fitBounds(full_extent@xmin, full_extent@ymin, full_extent@xmax, full_extent@ymax) %>%
        ...
        addLegend('topright', labels = c(paste0(spc, ' Automatic & Manual'), paste0(spc, ' Automatic Only'),
      ...
      m_range = leaflet() %>% addTiles() %>%
        ...
        addLegend('topright',labels = paste0(spc, ' Species Range'), colors = c('blue'), opacity =1) %>%
...
get_sa_results = function(
  ...
  sa_bulk_df_bats_only = subset(sa_bulk_df, sa_bulk_df$auto_id %in% proj_species_ids | sa_bulk_df$manual_id %in% proj_species_ids)
...
build_sa_table_3 = function(
  ...
  for (grts in all_grts){
    ...
    auto_species_names = unique(subset(proj_species_df, proj_species_df$species_code %in% auto_proj_species)$species)
...
build_sa_figure_1 = function(
  ...
  sa_figure_1 = get_grts_leaflet_map(all_grts = unique(subset(survey_df, survey_df$year == selected_year)$grts_cell_id),
...
build_sa_figure_2 = function(
  ...
  sa_descr_fig2a = paste0("Figure 2a. ",
    selected_year,
    " bat activity rate (average number of bat passes per night) by species. Species with at least one manual identification per site are shown in blue. Species identified only by automated identification software are shown in orange and species identified only by manual identification software are shown in green.")
  sa_descr_fig2b = paste0("Figure 2b. ",
    selected_year,
    " bat activity rate (average number of bat passes per night using a logarithmic scale) by species. Species with at least one manual identification per site are shown in blue. Species identified only by automated identification software are shown in orange and species identified only by manual identification software are shown in green.")
...
build_sa_figure_4 = function(
  ...
  proj_species_ids = unique(subset(proj_species_df, proj_species_df$species_code %in% proj_species_w_noid)$id)
  ...
  fig4_data = as.data.frame(table(species_only_df$grts_cell_id), stringsAsFactors = FALSE) %>%
