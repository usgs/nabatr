...
build_ac_doc = function(out_dir,
  ...
  for (grts in all_GRTS){
    ...
    this_grts_row = this_row_auto %>% dplyr::select(GRTS) %>%
      ...
      dplyr::mutate(`Species Detected` = number_species_grts)
  ...
  for (bat_spc in bat_species){
    types = unique(subset(table3_df, table3_df$`Species Detected` == subset(bats_df, bats_df$species_code == bat_spc)$species)$Method_of_Species_ID)
